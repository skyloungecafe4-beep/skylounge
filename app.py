...
app = Flask(__name__)
app.secret_key = "sky_lounge_kfc_style_key"

# Café ka Menu with Details & Images
menu_items = [
    {
        "id": 1, 
        "name": "Supreme Pizza", 
                ...
                <div class="bg-gray-900 rounded-xl overflow-hidden border border-gray-800 shadow-xl flex flex-col justify-between hover:border-red-600 transition">
                    <div>
                        ...
                        <div class="p-4">
                            ...
                            {% if item.has_sizes %}
                                <p class="text-red-400 font-bold text-sm mt-1">From Rs. {{ item.prices.Small }}</p>
                            {% else %}
...
# --- 2. KFC STYLE ITEM DETAIL POPUP PAGE ---
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden relative">
            ...
            <div class="p-6">
                ...
                <form action="/add-to-cart" method="POST" class="space-y-4">
                    ...
                    {% if item.has_sizes %}
                    <div>
                        <label class="block text-sm text-gray-400 mb-1 font-semibold">Select Size & Price</label>
                        <select name="size" class="w-full bg-gray-800 border border-gray-700 rounded-lg p-3 text-white text-sm focus:outline-none focus:border-red-600">
    ...
    if selected_item:
        ...
        for cart_item in session["cart"]:
            if cart_item["title"] == title:
                ...
                break
        if not found:
            session["cart"].append({"title": title, "price": price, "qty": qty})
...
def add_item():
    ...
    menu_items.append({"id": new_id, "name": name, "desc": "Delicious freshly prepared meal.", "has_sizes": False, "price": price, "image": default_img})
    return redirect(url_for("admin_dashboard"))
